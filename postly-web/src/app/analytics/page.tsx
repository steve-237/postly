  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-slate-900 to-indigo-900 rounded-3xl p-6 text-white shadow-xl shadow-indigo-900/20 md:col-span-1">
          ...
          <div className="mt-4 flex gap-4">
            <div>
              ...
              <p className="text-2xl font-bold text-indigo-300">{stats.last7}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">30 jours</p>
              <p className="text-2xl font-bold text-indigo-300">{stats.last30}</p>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 grid grid-cols-2 gap-4">
          {kpis.map((kpi) => {
            ...
            return (
              <div
                key={kpi.label}
                className={`bg-white rounded-3xl p-5 shadow-[0_4px_20px_rgb(0,0,0,0.04)] border ${kpi.border} flex items-center gap-4 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300`}
              >
      ...
      <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Activité des 7 derniers jours</h2>
            <p className="text-sm text-slate-500 mt-1 font-medium">Nombre de posts créés par jour</p>
          </div>
          <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full">
            <Activity className="w-4 h-4 text-slate-500" />
            <span className="text-xs font-bold text-slate-600">7 jours</span>
          </div>
        </div>
          ...
          <div className="flex items-end gap-3 h-48">
            {stats.dailyCounts.map((day) => (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs font-bold text-slate-600">{day.count > 0 ? day.count : ""}</span>
                <div className="w-full relative rounded-xl overflow-hidden bg-slate-100" style={{ height: "120px" }}>
                  <div
                    className="absolute bottom-0 w-full bg-gradient-to-t from-indigo-600 to-indigo-400 rounded-xl transition-all duration-700"
      ...
      {/* Répartition statuts */}
      <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8">
        <h2 className="text-xl font-bold text-slate-800 mb-6">Répartition par statut</h2>
        <div className="space-y-4">
          {[
            { label: "Publiés", count: stats.published, color: "bg-emerald-500", text: "text-emerald-700" },
            { label: "Planifiés", count: stats.scheduled, color: "bg-violet-500", text: "text-violet-700" },
            { label: "Brouillons", count: stats.draft, color: "bg-amber-400", text: "text-amber-700" },
            { label: "Échecs", count: stats.failed, color: "bg-red-500", text: "text-red-700" },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-4">
              <span className="w-24 text-sm font-bold text-slate-600 shrink-0">{item.label}</span>
